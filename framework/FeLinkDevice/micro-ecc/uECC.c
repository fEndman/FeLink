...
static uECC_word_t vli_testBit(const uECC_word_t *vli, bitcount_t b);
...
static int default_RNG(uint8_t *dest, unsigned count) {
    HCRYPTPROV prov;
    if (!CryptAcquireContext(&prov, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) {
        return 0;
    }

    CryptGenRandom(prov, count, (BYTE *)dest);
...
#define UNUSED(x) (x = x)
static int default_RNG(uint8_t *dest, unsigned size) FELINK_C51_REENTRANT {
    UNUSED(dest);
    UNUSED(size);
    return 0;
}
...
#if !asm_testBit
static uECC_word_t vli_testBit(const uECC_word_t *vli, bitcount_t b) {
    return (vli[b >> uECC_WORD_BITS_SHIFT] & ((uECC_word_t)1 << (b & uECC_WORD_BITS_MASK)));
}
#endif
...
int uECC_make_key(uint8_t public_key[uECC_BYTES*2], uint8_t private_key[uECC_BYTES]) {
    ...
    volatile uint8_t * data public_key_temp = public_key;
    volatile uint8_t * data private_key_temp = private_key;
#endif
